...
from tools.AzureAssistantTool import AzureAssistantTool
...
class MyAzureAssistantTool(MyTool):
    def __init__(self, tool_id=None):
        parameters = {} 
        super().__init__(
            tool_id,
            'AzureAIAssistantTool',
            "Azure AI assistant specialized in technical documentation and API knowledge. "
            "Answers questions about project architecture, API specifications, sem.NG framework, "
            "service endpoints, and infrastructure details. Leverages organizational knowledge bases "
            "and technical documentation to provide accurate, context-aware responses.",
            parameters
        )
    
    def create_tool(self) -> AzureAssistantTool:
        return AzureAssistantTool()
...
class MyCodeInterpreterTool(MyTool):
    def __init__(self, tool_id=None):
        ...
        super().__init__(
            tool_id, 
            'CodeInterpreterTool', 
            "This tool runs Python3 code in Docker.", 
            parameters
        )

    def create_tool(self) -> CodeInterpreterTool:
        return CodeInterpreterTool(
            docker_socket="npipe:////./pipe/docker_engine"  # Windows named pipe
        )

class MyCustomCodeInterpreterTool(MyTool):
    def __init__(self, tool_id=None, workspace_dir=None):
        parameters = {'workspace_dir': {'mandatory': False}}
        super().__init__(
            tool_id, 
            'CustomCodeInterpreterTool', 
            "Runs code in Docker with shared workspace.", 
            parameters, 
            workspace_dir=workspace_dir
        )

    def create_tool(self) -> CustomCodeInterpreterTool:
        return CustomCodeInterpreterTool(
            workspace_dir=self.parameters.get('workspace_dir', "workspace"),
            docker_socket="npipe:////./pipe/docker_engine"  # Windows named pipe
        )
...
TOOL_CLASSES = {
    'AzureAIAssistantTool': MyAzureAssistantTool,
